...
import { ImprovementRequestDoneButton } from "@/components/settings/improvement-request-done-button";
  ...
  return (
    <div className="space-y-4">
      {requests.map((request) => {
        const isDone = request.doneAt != null;

        return (
          <Card
            key={request.id}
            className={cn(isDone && "opacity-50 grayscale")}
          >
            <CardHeader className="space-y-3 pb-3">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="space-y-2">
                  <CardTitle className="text-base">{request.name}</CardTitle>
                  {isDone && request.doneAt && (
                    <Badge variant="secondary" className="font-normal">
                      Done · {formatDate(request.doneAt)}
                    </Badge>
                  )}
                </div>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {!isDone && (
                    <ImprovementRequestDoneButton requestId={request.id} />
                  )}
                </div>
              </div>
              <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">
                ...
                <span>投稿: {formatDateTime(request.createdAt)}</span>
              </div>
            </CardHeader>
            <CardContent>
